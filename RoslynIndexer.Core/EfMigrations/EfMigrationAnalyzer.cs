...
namespace RoslynIndexer.Core.Sql.EfMigrations
{
    ...
    public sealed class EfMigrationAnalyzer
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var file in files)
            {
                ...
                foreach (var cls in classes)
                {
                    ...
                    var info = AnalyzeMigrationClass(cls, file, repoRoot, cancellationToken);
                    if (info != null)
                        result.Add(info);
                }
            }
        ...
        private static EfMigrationOperation TryParseOperation(InvocationExpressionSyntax invocation)
        {
            ...
            // Schema.Table(nameof(Entity)) – existence checks / table touches
            ...
            if (string.Equals(parts[parts.Count - 1], "Sql", StringComparison.Ordinal))
            {
        ...
        private static List<string> FlattenMemberAccess(ExpressionSyntax expression)
        {
            ...
            {
                var id = ma.Name as IdentifierNameSyntax;
                if (id != null)
            ...
            var rootId = expression as IdentifierNameSyntax;
            if (rootId != null)
        ...
        private static string TryExtractName(ExpressionSyntax expression)
        {
                ...
                string.Equals(id.Identifier.Text, "nameof", StringComparison.Ordinal))
            {
                if (invocation.ArgumentList != null && invocation.ArgumentList.Arguments.Count > 0)
                {
                    var argExpr = invocation.ArgumentList.Arguments[0].Expression as IdentifierNameSyntax;
                    if (argExpr != null)
                        return argExpr.Identifier.Text;
                }
            }
    ...
    public sealed class EfMigrationOperation
    {
        ...
        /// <summary>
        /// Logical table name if the operation clearly targets a single table.
