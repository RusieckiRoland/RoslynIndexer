...
namespace RoslynIndexer.Tests.MixedIndexing
{
    [TestClass]
    [TestCategory("E2E")]
    public class MixedIndexing_RunnerE2ETests
    {
        [TestMethod]
        public void Should_Index_CSharp_And_Sql_FromRunner()
        {
            RunnerTestHelpers.EnsureWindowsOrInconclusive();
            string testRoot = Path.Combine(Path.GetTempPath(), "RI_E2E_MIX_" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(testRoot);

            try
            {
                Seeds.SeedMixedIndexing(testRoot, out var slnPath, out _, out var sqlPath);
