...
namespace RoslynIndexer.Tests.EfMigrations
{
    [TestClass]
    [TestCategory("E2E")]
    public class EfMigrations_RunnerE2ETests
    {
        [TestMethod]
        public void Should_Index_Migrations_FromRunner_ZipHasArtifacts()
        {
            RunnerTestHelpers.EnsureWindowsOrInconclusive();
            string testRoot = Path.Combine(Path.GetTempPath(), "RI_E2E_EF_MIG_" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(testRoot);

            try
            {
                Seeds.SeedEfMigrations(testRoot, out var slnPath, out _, out var efRoot);
                ...
                var (exit, stdout, stderr) =
                    RunnerTestHelpers.RunDotnet(
                        $"run -c Release --project \"{runnerProj}\" -- --config \"{cfg}\" --solution \"{slnPath}\" --temp-root \"{tempRoot}\" --ef \"{efRoot}\"",
